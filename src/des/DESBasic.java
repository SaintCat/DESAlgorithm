...
//            byte[] theKey = readBytes(keyFile);
            byte[] theKey = "asdzxcvb".getBytes();
            byte[][] subKeys = getSubkeys(theKey);
            byte[] theMsg = "qwertasdasdsadasdasdasdasaaayui".getBytes();
            byte[] theCph = cipher(theMsg, subKeys, "encrypt");
            byte[] the2 = cipher(theCph, subKeys, "decrypt");
//            writeBytes(theCph, cphFile);
            System.out.println(new String(the2));
            printBytes(theKey, "Key block");
            printBytes(theMsg, "Input block");
            printBytes(theCph, "Output block");
            System.out.println(new String(theMsg));
            System.out.println(new String(the2));
        } catch (Exception e) {
        }
    }
